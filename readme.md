...
OpenCart app for integrating Yespo with OpenCart.

## Purpose

The purpose of the plugin is to simplify the integration of OpenCart-based online stores with the Yespo platform and to provide all the necessary functionality without manual code intervention.

The plugin implements:

* Automatic transfer of current and historical customer data (contacts) from OpenCart to Yespo — including creation, deletion and update
* Automatic transfer of current and historical order data from OpenCart to Yespo — including creation and updates
...
6. Find **Yespo CDP** in the list and click the green **Install** button.
7. Click **Edit**.
8. Enter your **Full Access to API** key from your Yespo account and click **Synchronize**.
...
Authorization is performed using the Yespo API Key entered in the plugin settings.
...
* **Payload:** The plugin sends the store's domain, `serviceWorkerName` (e.g., `sw-yespo.js`), `serviceWorkerScope` (typically `/`), and the `serviceWorkerPath` (same as `serviceWorkerScope`).
...
| `CustomerData` | Customer registration, login, or order placement. Links site visitors to contacts in Yespo. Sent on any page after login and before `PurchasedItems`. |
...
* `syncedCount` — records successfully sent to Yespo.
* `failedCount` — records rejected by Yespo.
...
If a network error or API error occurs during synchronization, the process receives `ERROR` status. Data that was already successfully synchronized will not be re-sent after the error. Synchronization can be re-enabled in the plugin settings to resume from where it stopped.

For individual API requests (tracking script retrieval, Web Push registration, data batches), the plugin retries automatically up to 5 times with a 3-second delay between attempts. If all retries are exhausted, a **Try Again** button is displayed, allowing the administrator to resume the exact failed step without restarting the entire setup.

---

## Background Logging System

The plugin includes an isolated logging engine specifically for Yespo API interactions. It silently captures all connectivity issues, data validation errors, and file generation faults without exposing them to the frontend user. This provides developers with an actionable audit trail for debugging without affecting the store's conversion rates.

---

## Self-Check and Adaptation of the Yespo CDP Modifier

The Yespo CDP module uses OCMOD technology to safely inject code into your store. Since the modifier file uses the error="skip" parameter, the system skips problematic paths without throwing a critical error if there is a conflict with your custom theme or other modules.
...
3. Open the **Log** tab.
4. Press **Ctrl+F** and search for `yespo_cdp` or the modifier name used in your build.
...
* `catalog/view/theme/*/template/common/header*` — outputs the base Yespo scripts and usually searches for the `<body` tag.
...
3. Open the modifier file from the Yespo module.
...
2. Set Up Development Environment — follow the Installation Guide in [README.md](https://github.com/Yespo-CDP/yespo-opencart/blob/main/readme.md) to install the module in your OpenCart development environment.
...
2. Make Your Changes
   * Follow our [coding standards](https://github.com/Yespo-CDP/yespo-opencart/blob/main/readme.md#-coding-standards)
...
This project is licensed under the GNU General Public License v3.0 - see the [LICENSE](https://github.com/Yespo-CDP/yespo-opencart/blob/main/LICENSE.txt) file for details.
